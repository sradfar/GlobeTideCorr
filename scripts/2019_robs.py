...
#
# Description:
# This script performs an enhanced, full-period (1950–2019) co-variability
# analysis of major tidal constituents (M2, N2, S2, O1, K1, MK3, MS4) across the
# global tide-gauge network. For each station, annual maxima are extracted,
# primary and secondary constituents are identified, and Kendall τ correlations
# are computed between primary constituents and all others. Results are mapped
# globally using Robinson projection and summarized at the gauge level.
#
# Outputs:
# - Individual Robinson-projection maps for each tidal constituent (7 total),
#   including significant (p < 0.1) and non-significant correlations.
# - A global reference map of all gauge locations.
# - A comprehensive station-level significance report summarizing the number and
#   type of significant constituent pairs per gauge.
# - Diagnostic tables describing station counts, years of data, and correlation
#   distribution statistics.
#
# For methodological context and interpretation of global co-variability
# patterns, see:
...
for i, col in enumerate(columns):
    ...
    plot_gauges(dep_df_p, dep_df, col, "enhanced_1950-2019", subplot_label)
...
if significance_data:
    ...
    # Save to CSV
    significance_df.to_csv('gauge_significance_report.csv', index=False)
