...
#
# Description:
# This script produces a consolidated ENSO-enhanced correlation figure using a
# 7×3 Robinson-projection layout (7 tidal constituents × 3 ENSO phases). ONI
# data from NOAA are used to classify each year as El Niño, Neutral, or La Niña.
# Annual maxima of each constituent are extracted at all long-record tide gauges,
# primary/secondary constituents are identified, and Kendall τ correlations are
# computed separately for each ENSO phase. All subplots share a unified colorbar.
#
# Outputs:
# - A combined 21-panel Robinson-projection figure showing ENSO-conditioned
#   correlations for all major tidal constituents.
# - ENSO-specific summary tables and significance reports (p < 0.1).
# - Station-level diagnostics for the number of ENSO-conditioned years used.
#
# For scientific background and details of the global tidal co-variability
# framework, see:
...
# responsibility for inaccuracies, omissions, or outcomes resulting from its use.
# -----------------------------------------------------------------------------
...
def plot_single_subplot(ax, df_p, df_dep, col, enso_phase, row_idx, col_idx):
    """
    Plot a single subplot for ENSO analysis with Robinson projection
    """
    ...
    # Add subplot label only on the leftmost column
    if col_idx == 0:
        subplot_letters = ['a', 'b', 'c', 'd', 'e', 'f', 'g']
        subplot_label = subplot_letters[row_idx] if row_idx < len(subplot_letters) else f"({row_idx+1})"
        ax.text(-0.05, 1.05, f'{subplot_label}) {col}', transform=ax.transAxes, 
                fontsize=10, fontweight='bold', va='top', ha='left')
    
    # Add ENSO phase label only on the top row
    if row_idx == 0:
        ax.set_title(enso_phase, fontsize=12, fontweight='bold', pad=10)
    
    # Add minimal gridlines (no labels to save space)
...
# Create the single figure with subplots (7 rows x 3 columns)
print(f"\nCreating combined ENSO analysis figure with Robinson projection...")
...
for row_idx, col in enumerate(columns):
    for col_idx, enso_phase in enumerate(enso_phases):
        subplot_idx += 1
        
        # Create subplot with Robinson projection
        ax = fig.add_subplot(nrows, ncols, subplot_idx, projection=ccrs.Robinson())
        
        # Get data for this ENSO phase
        if enso_phase in enso_results:
            dep_df = enso_results[enso_phase]['dep_df']
            dep_df_p = enso_results[enso_phase]['dep_df_p']
            
            # Plot this subplot
            plot_single_subplot(ax, dep_df_p, dep_df, col, enso_phase, row_idx, col_idx)

from mpl_toolkits.axes_grid1 import make_axes_locatable
# Add a single colorbar for the entire figure
cbar_ax = fig.add_axes([0.15, 0.01, 0.68, 0.02])  # [left, bottom, width, height]
# Create a dummy scatter plot to get the colorbar
...
dummy_scatter = dummy_ax.scatter([], [], c=[], cmap='seismic', vmin=-1, vmax=1)
# cbar = fig.colorbar(dummy_scatter, ax=fig.get_axes(), orientation='horizontal', 
#                    pad=0.02, shrink=0.8, aspect=50)
cbar = plt.colorbar(dummy_scatter, cax=cbar_ax, orientation='horizontal')
cbar.set_label('Kendall τ', fontsize=14, fontweight='bold')
dummy_ax.remove()

# Adjust layout to prevent overlap
plt.tight_layout()
plt.subplots_adjust(bottom=0.05, top=0.98, hspace=0.05, wspace=0.05)

# Save the combined figure
fig.savefig("enso_robinson_combined_analysis.png", dpi=300, bbox_inches='tight', pad_inches=0.1)
plt.close()

# Print summary
print(f"\nENSO-enhanced correlation analysis complete!")
print(f"Generated combined Robinson projection figure with {len(columns)} constituents across {len(enso_phases)} ENSO phases.")
print("Figure saved as: enso_robinson_combined_analysis.png")
...
for col in columns:
    ...
    for enso_phase in enso_phases:
        if enso_phase in enso_results:
            ...
            if valid_corr.sum() > 0:
                mean_tau = dep_df[valid_corr][col].mean()
...
# Create significance reports for each ENSO phase
print(f"\nCreating ENSO-specific significance reports...")
...
for enso_phase in enso_phases:
    if enso_phase in enso_results:
        ...
        for idx, row in dep_df.iterrows():
            ...
            lat = row['lat']
            lon = row['lon']
            total_years = row['years']
            enso_years = row['enso_years']
        ...
        # Create DataFrame and save to CSV for this ENSO phase
        if significance_data:
            ...
            # Clean phase name for filename
            phase_name = enso_phase.replace(' ', '_').replace('ñ', 'n').lower()
            filename = f'gauge_significance_report_{phase_name}.csv'
            
            # Save to CSV
            significance_df.to_csv(filename, index=False)
            
            print(f"{enso_phase} report saved to '{filename}'")
            print(f"  - {len(significance_df)} gauges with significant constituents")
            print(f"  - {len(dep_df) - len(significance_df)} gauges excluded (no significant constituents)")
            
            # Print summary statistics for this phase
            print(f"  - Summary of significant constituents per gauge:")
            counts = significance_df['Num_Significant'].value_counts().sort_index()
            for num_sig, count in counts.items():
                print(f"    {num_sig} significant: {count} gauges")
        else:
            print(f"No gauges with significant constituents found for {enso_phase}")
        print()

print("All ENSO-specific significance reports created!")
